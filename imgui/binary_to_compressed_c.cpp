...
static bool
binary_to_compressed_c (const char *filename, const char *symbol,
			...
			bool use_static)
{
  ...
  if (source_encoding == SourceEncoding_Base85)
    {
      fprintf (
	out, "// Exported using binary_to_compressed_c.exe -base85 \"%s\" %s\n",
	filename, symbol);
...
static int
stb_compress_chunk (stb_uchar *history, stb_uchar *start, stb_uchar *end,
		    int length, int *pending_literals, stb_uchar **chash,
		    stb_uint mask)
{
  (void) history;
