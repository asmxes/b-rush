		     ...
		     float thickness, color col)
{
  ...
  float step = radius * 0.4f; // overlap circles
  ...
  for (size_t i = 0; i + 1 < points.size (); i++)
	...
	continue;
      ...
      int steps = static_cast<int> (len / step) + 1;
...
//
//   // for (size_t i = 0; i + 1 < points.size (); i++)
//   //   draw_list->AddLine (points[i], points[i + 1], col, thickness);
...
//       draw_list->AddCircleFilled (points.front (), thickness * 0.5f, col,
//       12); draw_list->AddCircleFilled (points.back (), thickness * 0.5f, col,
//       12); return;
//     }
//
...
//   --- std::vector<float> alphas (n_ma, 0.0f); for (int i = 1; i < n_ma - 1;
//   i++)
//     {
